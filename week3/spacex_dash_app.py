...
def get_pie_chart(entered_site):
    if entered_site == 'ALL':
        alldf = spacex_df.groupby('Launch Site')['class'].sum()
        fig = px.pie(alldf, values='class', 
        names=unique_values, 
        ...
        success_count = len(selected[selected['class'] == 1])
        failed_count = len(selected[selected['class'] == 0])
